...
## 2026-09-18 — Observability ticket 01: OTel metrics + `/metrics` endpoint
**Decision:** New `internal/telemetry` package exposes an OTel `MeterProvider` behind a Prometheus exporter (`/metrics`) and concord's instruments. The daemon serves `/metrics` on a **separate** listener from `CONCORD_METRICS_ADDR`; empty ⇒ observability off and coordination is byte-for-byte unchanged. Instruments: `concord_checkedit_total{decision}` (allowed|blocked_stale|blocked_no_read|allowed_new_file), `concord_rpc_duration_seconds{method}` histogram, `concord_overlaps_total`, `concord_divergences_total`, `concord_intent_expired_total`, `concord_events_dropped_total`, and a `concord_live_intents` gauge. RPC latency is recorded by a Connect **timing interceptor** (uniform across methods); decision/overlap/divergence counts are recorded inside the handlers after the verdict is computed (observe-only, ADR-0001).
**Why:** the decision label needs handler-level detail an interceptor can't see, but latency is uniform, so it belongs in one interceptor. A separate metrics port keeps telemetry off the coordination surface (a disabled-mode test asserts `/metrics` is never served on the RPC port). Metric names are chosen so the OTel Prometheus exporter's suffixing yields the exact series above (counter `concord_checkedit` → `_total`; histogram `concord_rpc_duration` with unit `s` → `_seconds`).
**Key sub-decisions:**
- **`live_intents` is an OTel *observable* gauge** whose callback reads the live intent count at scrape time. So the gauge falls when a record's TTL lapses with **no reaper** — consistent with ADR-0002. The callback is best-effort: a store error is swallowed and the scrape still succeeds (verified: the daemon serves `/metrics` with no Dragonfly).
- **`concord_events_dropped_total` and `concord_intent_expired_total` are registered now but emit a zero sample** (`Add(ctx, 0)`) so the metric surface is stable before the event plane wires their real increments (ticket 03/04). An OTel counter otherwise produces no series until first observation.
- **`NewService` stays backward-compatible** via a variadic `WithMetrics(Metrics)` option; the default is a no-op `Metrics`, so handlers call it unconditionally and the three existing `NewService(rs, rs)` call sites are untouched in behaviour.
- **`run()` takes an optional metrics `net.Listener`** (nil = disabled) instead of building it internally, so the daemon tests exercise both modes without guessing a port and stay Docker-free (they run in the Windows CI job).
- **Telemetry is tested through the existing RPC seam** (drive RPCs → scrape `/metrics` → assert series moved), so `internal/telemetry` has no direct tests by design — the metrics are asserted as external behaviour, matching the spec's testing decisions.
**Dependency bump:** OTel core `v1.44.0 → v1.46.0` to match `go.opentelemetry.io/otel/exporters/prometheus v0.68.0` + `sdk/metric v1.46.0`; adds `prometheus/client_golang v1.24.1` (transitive to the exporter).
**Verification:** `gofmt`/`vet`/`golangci-lint` clean; full suite green (`go test ./...`, real Dragonfly). Race detector could not run locally (Windows box, `CGO_ENABLED=0`, no gcc) — enforced by the CI ubuntu race job, as with prior tickets.
**Alternatives:** co-serving `/metrics` on the coordination mux (rejected — the ticket specifies a metrics *address*, and a separate port keeps telemetry off the RPC surface); incrementing `intent_expired` via a sweep/reaper (rejected — violates ADR-0002; expiry visibility comes from the gauge and, later, the `intent_expired` event).
**Links:** docs/specs/observability.md; `.scratch/observability/issues/01-*`; ADR-0002; ADR-0001; ADR-0007. ADR-0009 (observability architecture, best-effort trade) is recorded with the event plane (ticket 03).

## 2026-09-18 — Observability ticket 01: code-review follow-ups
**Decision:** Applied the ticket-01 review fixes (no critical items; "ready with fixes"):
1. **Divergence counter is query-relevant.** `QueryIntent` now increments `concord_divergences_total` only for records the query actually overlaps (nested under `PathOverlap`), matching the overlaps treatment. Both counters mean "surfaced by a query"; the true divergence/overlap **arrival-rate** signal will come from the event plane's `divergence_detected` / `overlap_reported` events (tickets 03/08). Prevents a polling orchestrator from re-counting the same standing divergent record on every poll and inflating the `rate()`.
2. **Stable metric surface.** `concord_overlaps_total` and `concord_divergences_total` are now zero-seeded at construction (like the backpressure counters), so a fresh daemon's Grafana panels read 0 rather than "no data".
3. **Bounded gauge callback.** The `live_intents` observable callback wraps its store round-trip in a 2s timeout so a hung Dragonfly stalls neither the scrape nor the collector goroutine.
4. **Test hardening.** `metricsFixtureTTL` now closes its store (`t.Cleanup`); the gauge test's TTL is 2s→5s (rise-poll headroom) with a 9s fall-poll (scrapes only read, so they never refresh TTL); `flushDragonfly` carries a loud warning that it is safe only while the package stays sequential (no `t.Parallel()`), with DB-index isolation named as the durable upgrade.
**Deferred (accepted):** ignored errors from OTel instrument constructors / `RegisterCallback` — OTel returns usable no-op instruments on error, so there is no nil-panic risk; surfacing them would require changing `NewMetrics` to return an error. Documented rather than fixed.
**Verification:** gofmt/vet/golangci-lint clean; full suite green (real Dragonfly). `-race` remains CI-enforced (no local gcc).
**Links:** DECISIONS "ticket 01"; superpowers:requesting-code-review pass; `.scratch/observability/issues/01-*`.

## 2026-09-18 — Observability ticket 02: local compose stack + Prometheus + Grafana-as-code
**Decision:** One compose topology under `deploy/observability/` — `docker compose up` starts concord (built from a minimal multi-stage distroless `Dockerfile`), Dragonfly, Prometheus, and Grafana. Prometheus scrapes the daemon's `/metrics` (`concord:9464`) from a committed scrape config; Grafana auto-provisions a Prometheus datasource (fixed uid `prometheus`) and the `concord-overview` dashboard from committed JSON — dashboards-as-code, no UI clicking. Ports: Grafana 3000, Prometheus 9090, concord RPC 8080 / metrics 9464.
**Verification gate — `make obs-smoke`:** a Go driver (`deploy/observability/smoke`) drives a stale `CheckEdit` + an overlapping intent query at the daemon, then asserts, with polling: `/metrics` exposes `concord_checkedit_total`, Prometheus reports `up{job="concord"}==1`, `concord_checkedit_total{decision="blocked_stale"}` and `concord_overlaps_total` are `>0` in Prometheus, and the `concord-overview` dashboard exists via the Grafana API. A shell wrapper (`scripts/obs-smoke.sh`) boots the stack `--build` and always `down -v` on exit. **Ran green locally** (full build + pulls + 6/6 assertions). `make obs-up`/`obs-down` added for hands-on use; documented in the README.
**Why a Go smoke driver, not bash+curl+jq:** reuses the generated Connect client to drive real coordination traffic, and avoids a `jq` dependency (parses the one Prometheus scalar inline). The stack's acceptance is thus asserted as external behaviour, same philosophy as the unit gate.
